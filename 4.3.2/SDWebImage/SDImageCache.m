...
// 从磁盘缓存中读取指定 key 对应的 image
- (nullable UIImage *)imageFromDiskCacheForKey:(nullable NSString *)key {
    UIImage *diskImage = [self diskImageForKey:key];
    if (diskImage && self.config.shouldCacheImagesInMemory) {
        //计算图片的像素点
        NSUInteger cost = SDCacheCostForImage(diskImage);
        //设置全局缓存实例时如果设置了totalCostLimit，存储缓存的方法调用必然带上了cost，否则totalCostLimit是无用的
...
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key options:(SDImageCacheOptions)options done:(nullable SDCacheQueryCompletedBlock)doneBlock {
    ...
    //这里的 operation 用来标记用来取消  没有用NSOperation的子类呀，取消有什么用？？？
    ...
    void(^queryDiskBlock)(void) =  ^{
        ...
        //创建 autorelease pool ？？？本次runloop迭代结束时清理掉被本次迭代期间被放到autorelease pool中的对象（有大量中间临时变量产生时，避免内存使用峰值过高，需要及时释放内存）
        @autoreleasepool {
